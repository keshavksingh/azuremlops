...
parser.add_argument("--SQL_READ_SPN", type=str, help="SQL_READ_SPN")
parser.add_argument("--SQL_READ_SPNKEY", type=str, help="SQL_READ_SPNKEY")
parser.add_argument("--SQL_SERVER_INSTANCE", type=str, help="SQL_SERVER_INSTANCE")
parser.add_argument("--AML_STORAGE_EXPERIMENT_SQL_ROOT_PATH", type=str, help="AML_STORAGE_EXPERIMENT_SQL_ROOT_PATH")
...
SQL_READ_SPN = args.SQL_READ_SPN
SQL_READ_SPNKEY = args.SQL_READ_SPNKEY
SQL_SERVER_INSTANCE = args.SQL_SERVER_INSTANCE
AML_STORAGE_EXPERIMENT_SQL_ROOT_PATH = args.AML_STORAGE_EXPERIMENT_SQL_ROOT_PATH
...
TenantId = "9493c38c-a42d-44cc-baef-40b3ee092c52"
...
Query = "SELECT * FROM [vw_iris_data]"

### Read From Azure SQL
df = read_from_azsql(SQL_SERVER_INSTANCE,access_token,Query,RUN_ID,PIPELINE_STEP_NAME)
## Write ADLS
write_to_adls_gen2(SOURCE_STORAGE_ACCOUNT_VALUE,\
                   AML_STORAGE_EXPERIMENT_SQL_ROOT_PATH,\
                   'parquet',\
                   1,\
                   df,\
                   AML_STORAGE_SPN_VALUE,\
                   AML_STORAGE_SPNKEY_VALUE,\
                   RUN_ID,\
                   PIPELINE_STEP_NAME)
...
source_v_id = LineageLogger.query_graph("g.V().hasLabel('amlrun').has('RUN_ID', '"+RUN_ID+"').has('PIPELINE_STEP_NAME', 'PreTrainDatabricksSparkKustoDataEngineeringStep').values('id')")[0]
